...
def test_get_direct_image_url_chnaged():
    ...
    direct_image_url = dcard_coworker.get_direct_image_url(image_url)
...
def test_find_image_urls_multiple():
    ...
    image_urls = dcard_coworker.find_image_urls(article_json)
...
def test_get_article_successs():
    session = aiohttp.ClientSession()
    article_id = '388072'
    result = yield from dcard_coworker.get_article(session, article_id)
    assert result != {}
